...
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
	...
	protected void drawOutlineExtracted(Rectangle2D dataArea, CategoryPlot plot, Graphics2D g2, double thisXOffset,
			double thisYOffset) {
		float x0 = (float) dataArea.getX();
		float x1 = x0 + (float) Math.abs(thisXOffset);
		float x3 = (float) dataArea.getMaxX();
	...
	protected void drawRangeGridline1(Graphics2D g2, CategoryPlot plot) {
	}

	protected void drawRangeGridlineExtracted(ValueAxis axis, double value, Rectangle2D dataArea, CategoryPlot plot,
			Graphics2D g2) {
		Range range = axis.getRange();
		if (!range.contains(value)) {
			return;
